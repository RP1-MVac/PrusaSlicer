...
///Lukáš Hejl @hejllukas, Tomáš Mészáros @tamasmeszaros, Pavel Mikuš @Godrak, David Kocík @kocikdav,
...
enum class ArcFittingType { Disabled, EmitCenter };
...
enum class ScarfSeamPlacement { nowhere, countours, everywhere };
...
enum DraftShield { dsDisabled, dsLimited, dsEnabled };

enum class LabelObjectsStyle { Disabled, Octoprint, Firmware };

enum class PerimeterGeneratorType {
    // Classic perimeter generator using Clipper offsets with constant extrusion width.
    Classic,
    // Perimeter generator with variable extrusion width based on the paper
    // "A framework for adaptive width control of dense contour-parallel toolpaths in fused
    // deposition modeling" ported from Cura.
    Arachne
};

enum class TopOnePerimeterType { None, TopSurfaces, TopmostOnly, Count };

enum class GCodeThumbnailsFormat { PNG, JPG, QOI };
...
// Defines each and every confiuration option of Slic3r, including the properties of the GUI
// dialogs. Does not store the actual values, but defines default values.
...
public:
    ...
    const std::vector<std::string> &extruder_retract_keys() const {
        return m_extruder_retract_keys;
    }
...
// The dynamic configuration is also used to store user modifications of the print global
// parameters, so the modified configuration values may be diffed against the active configuration
...
public:
    ...
    static DynamicPrintConfig full_print_config_with(
        const t_config_option_key &opt_key, const std::string &str, bool append = false
    ) {
    ...
    static DynamicPrintConfig new_with(
        const t_config_option_key &opt_key, const std::string &str, bool append = false
    ) {
    ...
    // Overrides ConfigBase::def(). Static configuration definition. Any value stored into this
    // ConfigBase shall have its definition here.
    ...
    // If the opt_key is no more valid in this version of Slic3r, opt_key is cleared by
    // handle_legacy(). handle_legacy() is called internally by set_deserialize().
...
public: \
    ...
    /* Overrides ConfigBase::keys(). Collect names of all configuration values maintained by this \
     * configuration store. */ \
...
private: \
    ...
    /* Cache object holding a key/option map, a list of option keys and a copy of this static \
     * config initialized with the defaults. */ \
    static StaticPrintConfig::StaticCache<CLASS_NAME> s_cache_##CLASS_NAME;

#define STATIC_PRINT_CONFIG_CACHE(CLASS_NAME) \
    STATIC_PRINT_CONFIG_CACHE_BASE(CLASS_NAME) \
public: \
    /* Public default constructor will initialize the key/option cache and the default object copy \
     * if needed. */ \
    CLASS_NAME() { \
        assert(s_cache_##CLASS_NAME.initialized()); \
...
#define PRINT_CONFIG_CLASS_ELEMENT_DEFINITION(r, data, elem) \
    BOOST_PP_TUPLE_ELEM(0, elem) BOOST_PP_TUPLE_ELEM(1, elem);
#define PRINT_CONFIG_CLASS_ELEMENT_INITIALIZATION2(KEY) \
    cache.opt_add(BOOST_PP_STRINGIZE(KEY), base_ptr, this->KEY);
...
#define PRINT_CONFIG_CLASS_ELEMENT_HASH(r, data, elem) \
    boost::hash_combine(seed, BOOST_PP_TUPLE_ELEM(1, elem).hash());
    ...
    protected: \
        void initialize(StaticCacheBase &cache, const char *base_ptr) { \
            BOOST_PP_SEQ_FOR_EACH( \
                PRINT_CONFIG_CLASS_ELEMENT_INITIALIZATION, _, PARAMETER_DEFINITION_SEQ \
            ) \
        } \
    };

#define PRINT_CONFIG_CLASS_DERIVED_CLASS_LIST_ITEM(r, data, i, elem) \
    BOOST_PP_COMMA_IF(i) public elem
#define PRINT_CONFIG_CLASS_DERIVED_CLASS_LIST(CLASSES_PARENTS_TUPLE) \
    BOOST_PP_SEQ_FOR_EACH_I( \
        PRINT_CONFIG_CLASS_DERIVED_CLASS_LIST_ITEM, _, BOOST_PP_TUPLE_TO_SEQ(CLASSES_PARENTS_TUPLE) \
    )
#define PRINT_CONFIG_CLASS_DERIVED_INITIALIZER_ITEM(r, VALUE, i, elem) \
    BOOST_PP_COMMA_IF(i) elem(VALUE)
#define PRINT_CONFIG_CLASS_DERIVED_INITIALIZER(CLASSES_PARENTS_TUPLE, VALUE) \
    BOOST_PP_SEQ_FOR_EACH_I( \
        PRINT_CONFIG_CLASS_DERIVED_INITIALIZER_ITEM, VALUE, \
        BOOST_PP_TUPLE_TO_SEQ(CLASSES_PARENTS_TUPLE) \
    )
#define PRINT_CONFIG_CLASS_DERIVED_INITCACHE_ITEM(r, data, elem) \
    this->elem::initialize(cache, base_ptr);
#define PRINT_CONFIG_CLASS_DERIVED_INITCACHE(CLASSES_PARENTS_TUPLE) \
    BOOST_PP_SEQ_FOR_EACH( \
        PRINT_CONFIG_CLASS_DERIVED_INITCACHE_ITEM, _, BOOST_PP_TUPLE_TO_SEQ(CLASSES_PARENTS_TUPLE) \
    )
...
#define PRINT_CONFIG_CLASS_DERIVED_DEFINE1( \
    CLASS_NAME, CLASSES_PARENTS_TUPLE, PARAMETER_DEFINITION, PARAMETER_REGISTRATION, \
...
\
    public: \
        ...
        size_t hash() const throw() { \
            size_t seed = 0; \
            BOOST_PP_SEQ_FOR_EACH( \
                PRINT_CONFIG_CLASS_DERIVED_HASH, _, BOOST_PP_TUPLE_TO_SEQ(CLASSES_PARENTS_TUPLE) \
            ) \
            PARAMETER_HASHES \
            return seed; \
        } \
        bool operator==(const CLASS_NAME &rhs) const throw() { \
            BOOST_PP_SEQ_FOR_EACH( \
                PRINT_CONFIG_CLASS_DERIVED_EQUAL, _, BOOST_PP_TUPLE_TO_SEQ(CLASSES_PARENTS_TUPLE) \
            ) \
            PARAMETER_EQUALS \
            return true; \
        } \
...
#define PRINT_CONFIG_CLASS_DERIVED_DEFINE0(CLASS_NAME, CLASSES_PARENTS_TUPLE) \
    PRINT_CONFIG_CLASS_DERIVED_DEFINE1( \
        CLASS_NAME, CLASSES_PARENTS_TUPLE, BOOST_PP_EMPTY(), BOOST_PP_EMPTY(), BOOST_PP_EMPTY(), \
        BOOST_PP_EMPTY() \
    )
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionFloat,
      support_material_spacing))((ConfigOptionFloat, support_material_speed))((ConfigOptionEnum<SupportMaterialStyle>, support_material_style))(
    ...
    // Tree supports
    ((ConfigOptionFloat,
      support_tree_angle))((ConfigOptionFloat, support_tree_angle_slow))((ConfigOptionFloat, support_tree_branch_diameter))((ConfigOptionFloat, support_tree_branch_diameter_angle))((ConfigOptionFloat, support_tree_branch_diameter_double_wall))((ConfigOptionPercent, support_tree_top_rate))((ConfigOptionFloat, support_tree_branch_distance))(
        (ConfigOptionFloat, support_tree_tip_diameter)
    )
    // The rest
    ((ConfigOptionBool,
      thick_bridges))((ConfigOptionFloat, xy_size_compensation))((ConfigOptionBool, wipe_into_objects))

        ((ConfigOptionBool,
          interlocking_beam))((ConfigOptionFloat, interlocking_beam_width))((ConfigOptionFloat, interlocking_orientation))((ConfigOptionInt, interlocking_beam_layer_count))((ConfigOptionInt, interlocking_depth))(
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionBool,
      automatic_infill_combination))((ConfigOptionFloatOrPercent, automatic_infill_combination_max_layer_height))((ConfigOptionFloat, bridge_angle))((ConfigOptionInt, bottom_solid_layers))((ConfigOptionFloat, bottom_solid_min_thickness))((ConfigOptionFloat, bridge_flow_ratio))((ConfigOptionFloat, bridge_speed))((ConfigOptionEnum<EnsureVerticalShellThickness>, ensure_vertical_shell_thickness))((ConfigOptionFloatOrPercent, over_bridge_speed))((ConfigOptionEnum<InfillPattern>, top_fill_pattern))((ConfigOptionEnum<InfillPattern>, bottom_fill_pattern))((ConfigOptionFloatOrPercent, external_perimeter_extrusion_width))((ConfigOptionFloatOrPercent, external_perimeter_speed))((ConfigOptionBool, enable_dynamic_overhang_speeds))((ConfigOptionFloatOrPercent, overhang_speed_0))((ConfigOptionFloatOrPercent, overhang_speed_1))((ConfigOptionFloatOrPercent, overhang_speed_2))((ConfigOptionFloatOrPercent, overhang_speed_3))((ConfigOptionBool, external_perimeters_first))((ConfigOptionBool, extra_perimeters))((ConfigOptionBool, extra_perimeters_on_overhangs))((ConfigOptionFloat, fill_angle))((ConfigOptionPercent, fill_density))((ConfigOptionEnum<InfillPattern>, fill_pattern))((ConfigOptionEnum<FuzzySkinType>, fuzzy_skin))((ConfigOptionFloat, fuzzy_skin_thickness))((ConfigOptionFloat, fuzzy_skin_point_dist))((ConfigOptionBool, gap_fill_enabled))((ConfigOptionFloat, gap_fill_speed))((ConfigOptionFloatOrPercent, infill_anchor))((ConfigOptionFloatOrPercent, infill_anchor_max))((ConfigOptionInt, infill_extruder))((ConfigOptionFloatOrPercent, infill_extrusion_width))((ConfigOptionInt, infill_every_layers))((ConfigOptionFloatOrPercent, infill_overlap))(
        (ConfigOptionFloat, infill_speed)
    )
    // Ironing options
    ((ConfigOptionBool,
      ironing))((ConfigOptionEnum<IroningType>, ironing_type))((ConfigOptionPercent, ironing_flowrate))((ConfigOptionFloat, ironing_spacing))(
        (ConfigOptionFloat, ironing_speed)
    )
    // Detect bridging perimeters
    ((ConfigOptionBool,
      overhangs))((ConfigOptionInt, perimeter_extruder))((ConfigOptionFloatOrPercent, perimeter_extrusion_width))(
        (ConfigOptionFloat, perimeter_speed)
    )
    // Total number of perimeters.
    ((ConfigOptionInt,
      perimeters))((ConfigOptionFloatOrPercent, small_perimeter_speed))((ConfigOptionFloat, solid_infill_below_area))((ConfigOptionInt, solid_infill_extruder))((ConfigOptionFloatOrPercent, solid_infill_extrusion_width))((ConfigOptionInt, solid_infill_every_layers))(
        (ConfigOptionFloatOrPercent, solid_infill_speed)
    )
    // Detect thin walls.
    ((ConfigOptionBool,
      thin_walls))((ConfigOptionFloatOrPercent, top_infill_extrusion_width))((ConfigOptionInt, top_solid_layers))((ConfigOptionFloat, top_solid_min_thickness))((ConfigOptionFloatOrPercent, top_solid_infill_speed))(
        (ConfigOptionBool, wipe_into_infill)
    )
    // Single perimeter.
    ((ConfigOptionEnum<TopOnePerimeterType>,
      top_one_perimeter_type))((ConfigOptionBool, only_one_perimeter_first_layer))

        ((ConfigOptionEnum<ScarfSeamPlacement>,
          scarf_seam_placement))((ConfigOptionBool, scarf_seam_only_on_smooth))((ConfigOptionPercent, scarf_seam_start_height))((ConfigOptionBool, scarf_seam_entire_loop))((ConfigOptionFloat, scarf_seam_length))((ConfigOptionFloat, scarf_seam_max_segment_length))(
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionFloats,
      machine_max_acceleration_x))((ConfigOptionFloats, machine_max_acceleration_y))((ConfigOptionFloats, machine_max_acceleration_z))(
        (ConfigOptionFloats, machine_max_acceleration_e)
    )
    // M203 X... Y... Z... E... [mm/sec]
    ((ConfigOptionFloats,
      machine_max_feedrate_x))((ConfigOptionFloats, machine_max_feedrate_y))((ConfigOptionFloats, machine_max_feedrate_z))(
        (ConfigOptionFloats, machine_max_feedrate_e)
    )

    // M204 P... R... T...[mm/sec^2]
    ((ConfigOptionFloats,
      machine_max_acceleration_extruding))((ConfigOptionFloats, machine_max_acceleration_retracting))(
        (ConfigOptionFloats, machine_max_acceleration_travel)
    )

    // M205 X... Y... Z... E... [mm/sec]
    ((ConfigOptionFloats,
      machine_max_jerk_x))((ConfigOptionFloats, machine_max_jerk_y))((ConfigOptionFloats, machine_max_jerk_z))(
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionEnum<ArcFittingType>,
      arc_fitting))((ConfigOptionBool, autoemit_temperature_commands))((ConfigOptionInt, bed_temperature_extruder))((ConfigOptionString, before_layer_gcode))((ConfigOptionString, between_objects_gcode))((ConfigOptionBool, binary_gcode))((ConfigOptionFloats, deretract_speed))((ConfigOptionString, end_gcode))((ConfigOptionStrings, end_filament_gcode))((ConfigOptionString, extrusion_axis))((ConfigOptionFloats, extrusion_multiplier))((ConfigOptionFloats, filament_diameter))((ConfigOptionFloats, filament_density))((ConfigOptionStrings, filament_type))((ConfigOptionBools, filament_soluble))((ConfigOptionBools, filament_abrasive))((ConfigOptionFloats, filament_cost))((ConfigOptionFloats, filament_spool_weight))((ConfigOptionFloats, filament_max_volumetric_speed))((ConfigOptionFloats, filament_infill_max_speed))((ConfigOptionFloats, filament_infill_max_crossing_speed))((ConfigOptionFloats, filament_loading_speed))((ConfigOptionFloats, filament_loading_speed_start))((ConfigOptionFloats, filament_load_time))((ConfigOptionFloats, filament_unloading_speed))((ConfigOptionFloats, filament_unloading_speed_start))((ConfigOptionFloats, filament_toolchange_delay))((ConfigOptionFloats, filament_unload_time))((ConfigOptionInts, filament_cooling_moves))((ConfigOptionFloats, filament_cooling_initial_speed))((ConfigOptionFloats, filament_minimal_purge_on_wipe_tower))((ConfigOptionFloats, filament_cooling_final_speed))((ConfigOptionPercents, filament_purge_multiplier))((ConfigOptionStrings, filament_ramming_parameters))((ConfigOptionBools, filament_multitool_ramming))((ConfigOptionFloats, filament_multitool_ramming_volume))((ConfigOptionFloats, filament_multitool_ramming_flow))((ConfigOptionFloats, filament_stamping_loading_speed))((ConfigOptionFloats, filament_stamping_distance))((ConfigOptionFloatsOrPercentsNullable, filament_seam_gap_distance))((ConfigOptionPercents, filament_shrinkage_compensation_xy))((ConfigOptionPercents, filament_shrinkage_compensation_z))((ConfigOptionBool, gcode_comments))((ConfigOptionEnum<GCodeFlavor>, gcode_flavor))(
...
PRINT_CONFIG_CLASS_DERIVED_DEFINE(
    ...
    ((ConfigOptionBool,
      automatic_extrusion_widths))((ConfigOptionBool, adjust_layer_height_to_match))((ConfigOptionBool, avoid_crossing_curled_overhangs))((ConfigOptionBool, avoid_crossing_perimeters))((ConfigOptionFloatOrPercent, avoid_crossing_perimeters_max_detour))((ConfigOptionPoints, bed_shape))((ConfigOptionInts, bed_temperature))((ConfigOptionFloat, bridge_acceleration))((ConfigOptionInts, bridge_fan_speed))((ConfigOptionBools, enable_dynamic_fan_speeds))((ConfigOptionInts, overhang_fan_speed_0))((ConfigOptionInts, overhang_fan_speed_1))((ConfigOptionInts, overhang_fan_speed_2))((ConfigOptionInts, overhang_fan_speed_3))((ConfigOptionInts, chamber_temperature))((ConfigOptionInts, chamber_minimal_temperature))((ConfigOptionBool, complete_objects))((ConfigOptionFloats, colorprint_heights))((ConfigOptionBools, cooling))((ConfigOptionEnums<CoolingSlowdownLogicType>, cooling_slowdown_logic))((ConfigOptionFloats, cooling_perimeter_transition_distance))((ConfigOptionFloat, default_acceleration))((ConfigOptionInts, disable_fan_first_layers))((ConfigOptionEnum<DraftShield>, draft_shield))((ConfigOptionFloat, duplicate_distance))((ConfigOptionFloat, external_perimeter_acceleration))((ConfigOptionFloat, extruder_clearance_height))((ConfigOptionFloat, extruder_clearance_radius))((ConfigOptionStrings, extruder_colour))((ConfigOptionPoints, extruder_offset))((ConfigOptionBools, fan_always_on))((ConfigOptionInts, fan_below_layer_time))((ConfigOptionStrings, filament_colour))((ConfigOptionStrings, filament_notes))((ConfigOptionFloat, first_layer_acceleration))((ConfigOptionInts, first_layer_bed_temperature))((ConfigOptionFloatOrPercent, first_layer_extrusion_width))((ConfigOptionFloatOrPercent, first_layer_height))((ConfigOptionFloatOrPercent, first_layer_speed))((ConfigOptionFloatOrPercent, first_layer_infill_speed))((ConfigOptionInts, first_layer_temperature))((ConfigOptionIntsNullable, idle_temperature))((ConfigOptionInts, full_fan_speed_layer))((ConfigOptionFloat, infill_acceleration))((ConfigOptionBool, infill_first))((ConfigOptionInts, max_fan_speed))((ConfigOptionFloats, max_layer_height))((ConfigOptionInts, min_fan_speed))((ConfigOptionFloats, min_layer_height))((ConfigOptionFloat, max_print_height))((ConfigOptionFloats, min_print_speed))((ConfigOptionFloat, min_skirt_length))((ConfigOptionString, notes))((ConfigOptionString, custom_parameters_print))((ConfigOptionFloats, nozzle_diameter))((ConfigOptionBool, only_retract_when_crossing_perimeters))((ConfigOptionBool, ooze_prevention))((ConfigOptionString, output_filename_format))((ConfigOptionFloat, perimeter_acceleration))((ConfigOptionStrings, post_process))((ConfigOptionBool, prefer_clockwise_movements))((ConfigOptionString, printer_model))((ConfigOptionString, printer_notes))((ConfigOptionFloat, resolution))((ConfigOptionFloat, gcode_resolution))((ConfigOptionFloats, retract_before_travel))((ConfigOptionBools, retract_layer_change))((ConfigOptionFloat, skirt_distance))((ConfigOptionInt, skirt_height))((ConfigOptionInt, skirts))((ConfigOptionInts, slowdown_below_layer_time))((ConfigOptionFloat, solid_infill_acceleration))((ConfigOptionBool, spiral_vase))((ConfigOptionInt, standby_temperature_delta))((ConfigOptionInts, temperature))((ConfigOptionInt, threads))((ConfigOptionString, thumbnails))((ConfigOptionEnum<GCodeThumbnailsFormat>, thumbnails_format))((ConfigOptionFloat, top_solid_infill_acceleration))((ConfigOptionFloat, travel_acceleration))((ConfigOptionFloat, travel_short_distance_acceleration))((ConfigOptionBools, wipe))((ConfigOptionBool, wipe_tower))((ConfigOptionFloat, wipe_tower_acceleration))((ConfigOptionFloat, wipe_tower_width))((ConfigOptionFloat, wipe_tower_per_color_wipe))((ConfigOptionFloat, wipe_tower_brim_width))((ConfigOptionFloat, wipe_tower_cone_angle))((ConfigOptionPercent, wipe_tower_extra_spacing))((ConfigOptionPercent, wipe_tower_extra_flow))((ConfigOptionFloat, wipe_tower_bridging))((ConfigOptionInt, wipe_tower_extruder))((ConfigOptionFloats, wiping_volumes_matrix))((ConfigOptionBool, wiping_volumes_use_custom_matrix))(
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionEnum<PrinterTechnology>,
      printer_technology))((ConfigOptionPoints, bed_shape))((ConfigOptionFloat, max_print_height))((ConfigOptionFloat, display_width))((ConfigOptionFloat, display_height))((ConfigOptionInt, display_pixels_x))((ConfigOptionInt, display_pixels_y))((ConfigOptionEnum<SLADisplayOrientation>, display_orientation))((ConfigOptionBool, display_mirror_x))((ConfigOptionBool, display_mirror_y))((ConfigOptionFloats, relative_correction))((ConfigOptionFloat, relative_correction_x))((ConfigOptionFloat, relative_correction_y))((ConfigOptionFloat, relative_correction_z))((ConfigOptionFloat, absolute_correction))((ConfigOptionFloat, elefant_foot_compensation))((ConfigOptionFloat, elefant_foot_min_width))((ConfigOptionFloat, gamma_correction))((ConfigOptionFloat, fast_tilt_time))((ConfigOptionFloat, slow_tilt_time))(
        (ConfigOptionFloat, high_viscosity_tilt_time)
    )
    //    ((ConfigOptionFloat,                      area_fill))
    ((ConfigOptionFloat,
      min_exposure_time))((ConfigOptionFloat, max_exposure_time))((ConfigOptionFloat, min_initial_exposure_time))((ConfigOptionFloat, max_initial_exposure_time))((ConfigOptionString, sla_archive_format))((ConfigOptionFloat, sla_output_precision))(
        (ConfigOptionString, printer_model)
    )
)

PRINT_CONFIG_CLASS_DERIVED_DEFINE0(
    SLAFullPrintConfig, (SLAPrinterConfig, SLAPrintConfig, SLAPrintObjectConfig, SLAMaterialConfig)
)
...
// This class defines the command line options representing actions including options representing
// profiles sharing commands.
...
public:
    ...
    void apply_only(
        const ModelConfig &other, const t_config_option_keys &keys, bool ignore_nonexistent = false
    ) {
        this->apply_only(other.get(), keys, ignore_nonexistent);
    }
    void apply_only(
        const ConfigBase &other, const t_config_option_keys &keys, bool ignore_nonexistent = false
    ) {
    ...
    std::string opt_serialize(const t_config_option_key &opt_key) const {
        return m_data.opt_serialize(opt_key);
    }
...
namespace cereal {
// Let cereal know that there are load / save non-member functions declared for DynamicPrintConfig,
// ignore serialize / load / save from parent class DynamicConfig.
...
template<class Archive> void load(Archive &archive, Slic3r::DynamicPrintConfig &config) {
    ...
    for (size_t i = 0; i < cnt; ++i) {
        ...
        auto it = Slic3r::print_config_def.by_serialization_key_ordinal.find(
